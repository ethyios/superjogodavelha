...
**Opção "Jogo Rápido ⚡":** Toggle disponível na tela de configuração de partida. Ao lado, um ícone **"?"** exibe um tooltip flutuante (popup que aparece enquanto pressionado/hover) com a explicação: _"Desativa o tempo mínimo de espera da IA (0.5s). A IA jogará instantaneamente."_ O valor é passado à Engine como `configuracoes.delay_minimo_ia` no `MatchPayload`.
